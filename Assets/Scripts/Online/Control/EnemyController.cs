...
namespace Online
{
    public class EnemyController : Controller, IEnemyController
    {
        ...
        {
            if (targetPlayer == null || targetPlayer.GetComponent<IState>().health <= 0 || Time.time - lastFindTargetTime > 1f) // 没有当前目标或当前目标死亡或距上次找目标超过了1秒
            {
        ...
        private GameObject FindNearestPlayer()
        {
            ...
            {
                if (player.GetComponent<IState>().health > 0)
                {
