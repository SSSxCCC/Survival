...
namespace Offline
{
    public class EnemyWeapon : MonoBehaviour, IEnemyWeapon
    {
        ...
        private void Awake() { commonEnemyWeapon = new CommonEnemyWeapon<EnemyWeapon>(this); }

        // 初始化引用
        private void Start() { commonEnemyWeapon.Start(); }

        // 自动射击
        private void Update() { commonEnemyWeapon.CreateAmmo(); }
    }
}