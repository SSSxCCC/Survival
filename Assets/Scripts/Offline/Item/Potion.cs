...
namespace Offline
{
    public class Potion : MonoBehaviour, IPotion
    {
        ...
        private void Awake() { commonPotion = new CommonPotion<Potion>(this); }

        // 被玩家捡到增加相应状态属性
        private void OnTriggerEnter2D(Collider2D collider) { if (commonPotion.FindOwner(collider)) Destroy(gameObject); }
    }
}